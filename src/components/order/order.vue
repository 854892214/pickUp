  ...
  <div>
      ...
      <div class="mui-content mui-row mui-fullscreen">
          ...
          <div class="goods-control-content" :id="'item_'+i" v-for="(item,i) in list" :key="i">
            ...
            <div class="goods-item" v-for="(goods ,j) in item.goodsList" :key="j"
              ...
              <div class="goods-item-content" :data-meun_index="i" :data-goods_index="j">
                ...
                <div class="spec-btn" v-if="goods.hasOwnProperty('specificationInfoList')" :data-meun_index="i"
                     :data-goods_index="j" @click="selectSpecClick">选规格
                </div>
                <div class="cart-btn" v-if="goods.hasOwnProperty('specificationInfoList') === false">
                  <span class="icon-remove_circle_outline" :data-goods_id="goods.id" v-show="goods.num>0"
                        @click="removeCount"></span>
        ...
        <div class="shopcart" :class="shopCartCount > 0 ? 'active' : ''"
          ...
          <div class="content-right" :class="shopCartPrice > 0 ? 'active' : ''">
            <div class="pay-btn" @click="clickPayment">{{this.$store.getters.getIsOrder === 'TRUE' ? '加单' : '去下单'}}
            </div>
          </div>
        </div>
      </div>
    ...
    <mt-popup v-model="remarkPopupVisible" :closeOnClickModal="remarkModal" popup-transition="popup-fade"
      ...
      <div class="remark-hreader">
        ...
        <div class="close" @click="remarkPopupVisible = false"><span class="mui-icon mui-icon-closeempty"></span></div>
      </div>
      <div class="remark-content">
        <div class="item-list">
          <div class="item" @click="selectRemark" :data-index="i" v-for="(remarkInfo,i) of remarkList" :key="i"
               :class="remarkInfo.active ? 'active' : ''">{{remarkInfo.name}}
          </div>
        </div>
      </div>
        ...
        <ul>
          <li class="food" v-for="(item,i) of shopCartList" :key="i">
            ...
            <div class="list-food-float-right">
              ...
              <div class="cart-btn">
                <span class="icon-remove_circle_outline" :data-index=i @click="shopCartRemoveGoods"></span>
    ...
    <!--<nav class="mui-bar mui-bar-tab">-->
      <!--<a class="mui-tab-item" @click="jumpTab(1)" :class="this.$store.getters.getTabbarStatus==1?'mui-active':''">-->
        <!--<svg class="mui-icon icon" aria-hidden="true">-->
          <!--<use xlink:href="#icon-shouye"></use>-->
        <!--</svg>-->
        <!--<span class="mui-tab-label">首页</span>-->
      <!--</a>-->
      <!--<a class="mui-tab-item" @click="jumpTab(2)" :class="this.$store.getters.getTabbarStatus==2?'mui-active':''">-->
        <!--<svg class="mui-icon icon" aria-hidden="true">-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //库存检测
      checkQuantity(goodsId, type) {
        // let flag = false;
        // let quantity = 0;
        // let where = {"id" : goodsId};
        // if(this.db(where).first()){
        //     quantity = this.db(where).first().count;
        //     if(type === "ADD"){
        //       quantity  = quantity + 1;
        //     }else if(type === "REMOVE"){
        //       quantity  = quantity - 1;
        ...
        //     if (goodsId == goods.id ) {
        //       if(goods.inventory_status == 1 && goods.quantity >= quantity){
        //         flag = true;
        //       }else if(goods.inventory_status == 0){
        //         flag = true;
        //       }
        //     }
        //   }
      ...
      specAddCart() {
        ...
        console.log(selectSpecItemList)
      ...
      updateShopCart() {
        ...
        this.db().each((result) => {
          console.log(result)
          let goodsTotalAmount
          if(result.discount==undefined||result.discount==""){
            goodsTotalAmount = this.common.round((result.amount * result.count), 2);
          }else {
            goodsTotalAmount = this.common.round((parseFloat((result.amount * result.discount).toFixed(2)) * result.count), 2);
          }
          if (result.specItem != "") {
      ...
      //底部tabbar路由跳转
      jumpTab(item){
        this.$store.commit('tabbarStatus',item)
        if(this.$store.getters.getTabbarStatus==1){
          this.$router.push('/order')
        }else if(this.$store.getters.getTabbarStatus==2){
