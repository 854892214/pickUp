<script src="../../../../堂食/src/assets/js/keyboard.js"></script>
      ...
      <div class="mui-content">
        <div class="info">
          ...
          <div class="total-amount" v-if="!isDeduction">
            ${{(totalAmount.toFixed(2)-transfromPrice(integral)).toFixed(2)}}
          </div>
          <div class="total-amount" v-if="isDeduction">${{0.01}}</div>
          <!--<div class="total" v-if="!isDeduction">：<span class="price">${{(totalAmount.toFixed(2)-transfromPrice(integral)).toFixed(2)}}</span>-->
          <!--</div>-->
          <!--<div class="total" v-if="isDeduction">合计：<span class="price">${{((totalAmount.toFixed(2)-(totalAmount-0.01))).toFixed(2)}}</span>-->
          <!--</div>-->
          <div class="desk-no">
            <span>到店自取</span>
          ...
          <div class="goods-item" v-for="goods of goodsList">
            ...
            <div class="group-amount">${{goods.total_amount.toFixed(2)}}</div>
          </div>
          <div class="cost-info">
            <div class="item">
              <p>小计</p>
          ...
          <!--((totalAmount.toFixed(2)-transfromPrice(integral)).toFixed(2)-discountAmount).toFixed(2)-->
          ...
          <!-- v-if="this.$store.getters.getIsStoreUser=='FALSE'"-->
          <p class="register-prompt" @click="jumpRegister" v-if="this.$store.getters.getIsStoreUser=='FALSE'">
            (注册会员使用积分抵扣)</p>
        </div>
        <!--会员积分抵扣-->
        <!--<div class="info" v-if="this.$store.getters.getOpenMembershipSystem=='TRUE'&&this.$store.getters.getIsStoreUser=='TRUE'">-->
          ...
          <div class="cost-border">
            <div class="item">
              ...
              <div class="consumption-info">
                您当前消费<span>{{((totalAmount.toFixed(2)-transfromPrice(integral))).toFixed(2)}}</span>,可获得 <span>{{Math.round(transfromIntegral(((totalAmount.toFixed(2)-transfromPrice(integral)))))}}</span>积分
              </div>
            </div>
          </div>
        ...
        <div class="mint-msgbox-content">
          <div class="mint-msgbox-message">
            ...
            <span style="font-weight: 600;">$
                  <span
                    style="padding: 0 0px;" v-if="this.$store.getters.getOpenMembershipSystem=='TRUE'">{{transRemainder(this.$store.getters.getNowIntegralNumber).toFixed(2)}}</span>
                </span>
          </div>
...
<script>
  require("../../../../堂食/src/assets/js/keyboard")
  ...
  export default {
    ...
    data() {
      return {
        ...
        //订单未支付且扣了积分的当前积分数
        nowAsBeforeIntegra: +(this.$store.getters.getDeductionIntegral) + (+(this.$store.getters.getNowIntegralNumber)),//+(this.$store.getters.getDeductionIntegral) + (+(this.$store.getters.getNowIntegralNumber))
        //是否注册了会员
    ...
    methods: {
      ...
      addOrder() {
        ...
        let url = this.common.baseURL + "mobi/goodsorder/add?store_id=" + this.$store.getters.getStoreId + "&store_menu_id=" + this.$store.getters.getMenuId;
        if (this.$store.getters.getIsStoreUser === 'TRUE') {
          url += "&store_user_id=" + this.$store.getters.getStoreScanGoodsUser.id;
        }
        ...
        if (this.$store.getters.getGoodsOrderNum === '' || this.$store.getters.getGoodsOrderNum == null) {
          this.axios.post(url, {
            ...
            remark: userRemark,
            tack_out_time: this.common.toTimeZone(this.$store.getters.getToTime, this.$store.getters.getTimeZone, 'UTC')
          }).then(result => {
      ...
      payment(orderNum, tip, paytype) {
        ...
        if (this.$store.getters.getIsStoreUser == "TRUE") {
          ...
          // console.log(this.$store.getters.getStoreScanGoodsUser.id,this.$store.getters.getStoreId)
        ...
        this.axios.post(url, body).then(result => {//*
          if (result.data.result_code == "ok" && result.data.result_msg == "SUCCESS") {
            // let nowIntegral=this.$store.getters.getNowIntegralNumber;
      ...
      beforeComputed(goodsTotalAmount, discount, isTax, taxRate, isPreDiscountTax) {
        ...
        baseAmount = this.common.round(baseAmount, 2)
      ...
      getComputed() {
        ...
        let discount = +(this.$store.getters.getDiscount)
        let isTax = this.$store.getters.getIsTax == "TRUE"
        let taxRate = +(this.$store.getters.getTaxRate)
        let isDiscountPreRax = this.$store.getters.getIsAfterDiscountTax == "TRUE" ? true : false
    ...
    created() {
      this.toTimeShowText = this.common.dateComputed(this.$store.getters.getToTime, this.$store.getters.getTimeZone);
      if (this.$store.getters.getOpenMembershipSystem == "TRUE") {
      ...
      this.goodsList = this.$store.getters.getShopCartList;
      for (let goods of this.goodsList) {
        this.total += goods.total_amount;
      }
      this.getComputed();
    ...
    mounted() {
      ...
      if (this.$store.getters.getOpenMembershipSystem == "TRUE" && this.$store.getters.getIsStoreUser == "TRUE") {
        ...
        // this.$store.getters.getDeductionIntegral > 0 ? this.integralHTML = this.$store.getters.getDeductionIntegral + "($" + this.transfromPrice(this.$store.getters.getDeductionIntegral) + ")" : ""
    ...
    watch: {
      ...
      tipItem: {
        handler: function (newVal, oldVal) {
          ...
          let isPreTax = this.$store.getters.getTipType == "PRE_TAX"
